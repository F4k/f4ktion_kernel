			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s value", __func__);
	ret_1 = snd_soc_read(codec, TAPAN_A_CDC_TX1_VOL_CTL_GAIN);
	ret_2 = snd_soc_read(codec, TAPAN_A_CDC_TX3_VOL_CTL_GAIN);

	if( ret_1 == 0 && ret_2 == 0)
	{
		pr_info(" %s ret_1 = %d ret_2 = %d ", __func__,ret_1,ret_2);
